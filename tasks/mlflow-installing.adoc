...
* A {kube} namespace for {productname} components (referred to as `SAIF_NAMESPACE` below).
...
* `SAIF_NAMESPACE` -- the {kube} namespace where {productname} components are deployed.
...
[source,,subs="+attributes"]
----
{prompt_user}kubectl get pods -n SAIF_NAMESPACE
...
[source,,subs="+attributes"]
----
{prompt_user}kubectl logs postgresql-0 -n SAIF_NAMESPACE
----
+
...
[source,,subs="+attributes"]
----
{prompt_user}kubectl get secret -n SAIF_NAMESPACE postgresql \
...
[source,,subs="+attributes"]
----
{prompt_user}kubectl exec -it postgresql-0 -n SAIF_NAMESPACE -- bash
----
...
backendStoreUri: postgresql+psycopg2://appuser:$(MLFLOW_DB_PASSWORD)@postgresql.SAIF_NAMESPACE.svc.cluster.local:5432/appdb
...
[source,,subs="+attributes"]
----
{prompt_user}kubectl logs deployment/mlflow -n SAIF_NAMESPACE
----
+
Expected output:
+
...
[source,,subs="+attributes"]
----
{prompt_user}kubectl get ingress -n SAIF_NAMESPACE
