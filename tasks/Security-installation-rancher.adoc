...
Deploying {ssecurity} through {ranchera} integrates advanced container security, vulnerability scanning, and runtime protection directly into cluster management workflows.
This integration enables single sign-on (SSO) console access and centralized security policy control across managed environments.
Deploy this component using {ranchera} Extensions or Apps & Marketplace to install security containers into the default namespace.
endif::[]
