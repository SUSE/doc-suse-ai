...
Alternatively, include a link:https://ranchermanager.docs.rancher.com/integrations-in-rancher/neuvector/configuration/configmaps#protect-sensitive-data-using-a-secret[ConfigMap as a secret][ConfigMap as a secret] in the initial deployment from {ranchera} (see chart values for ConfigMap settings) to set the default admin password to a strong password.
====
