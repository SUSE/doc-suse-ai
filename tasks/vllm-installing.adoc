...
Find examples of custom values files in xref:vllm-helm-overrides[].
. After saving the custom values file as `vllm_custom_overrides.yaml`, apply its configuration with the following command.
+
...
[source,bash,subs="+attributes"]
----
{prompt_user}kubectl get deployments -n <SAIF_NAMESPACE>
...
----
{prompt_user}kubectl get deployments -n <SAIF_NAMESPACE>
{prompt_user}kubectl delete deployment vllm-deployment-router -n <SAIF_NAMESPACE>
{prompt_user}kubectl delete deployment <MODEL_DEPLOYMENT_NAME> -n <SAIF_NAMESPACE>
----
...
[source,bash,subs="+attributes"]
----
{prompt_user}helm uninstall vllm -n <SAIF_NAMESPACE>
----
