...
{sobservability} integrated with the {suseai} Observability Extension monitors and analyzes enterprise artificial intelligence (AI) workloads across cluster environments.
Tracking metrics, logs and traces in real time enables proactive performance tuning, simplifies troubleshooting and maintains optimal cluster health.
Deploy this solution in single-cluster or multi-cluster environments to establish unified telemetry pipelines.
endif::[]
      ...
      config:
        scrape_configs:
          ...
          - job_name: 'milvus'
            ...
            static_configs:
              - targets: ['<MILVUS_SERVICE_NAME>.<SAIF_NAMESPACE>.svc.cluster.local:9091'] <.>
          - job_name: 'qdrant'
            scrape_interval: 10s
            metrics_path: '/metrics'
            static_configs:
              - targets: ['<QDRANT_SERVICE_NAME>.<SAIF_NAMESPACE>.svc.cluster.local:6333'] <.>
          - job_name: 'vllm'
            scrape_interval: 10s
            scheme: http
            kubernetes_sd_configs:
      ...
      metric_statements:
        - context: resource
          statements:
            ...
            - set(attributes["k8s.namespace.name"], "${env:SAIF_NAMESPACE}") where attributes["service.name"] == "vllm"
    transform/qdrant:
      error_mode: ignore
      metric_statements:
        - context: resource
          statements:
            ...
            - set(attributes["k8s.namespace.name"], "${env:SAIF_NAMESPACE}") where attributes["service.name"] == "qdrant"
    transform/milvus:
      error_mode: ignore
      metric_statements:
        - context: resource
          statements:
            ...
            - set(attributes["k8s.namespace.name"], "${env:SAIF_NAMESPACE}") where attributes["service.name"] == "milvus"
    tail_sampling:
      decision_wait: 10s
      policies:
      ...
      metric_statements:
        - context: resource
          statements:
            ...
            - set(attributes["k8s.namespace.name"], "${env:SAIF_NAMESPACE}") where attributes["gen_ai.provider.name"] != nil
      ...
      metric_statements:
        - context: resource
          statements:
            ...
            - set(attributes["k8s.namespace.name"], "${env:SAIF_NAMESPACE}") where attributes["gen_ai.request.model"] != nil
      ...
      metric_statements:
        - context: resource
          statements:
            ...
            - set(attributes["k8s.namespace.name"], "${env:SAIF_NAMESPACE}")
...
<.> Replace `<SAIF_NAMESPACE>` with the namespace where {productname} components are installed.
...
--
. *Install {productname}.*
Refer to xref:aif-deployment[] for the complete procedure.
