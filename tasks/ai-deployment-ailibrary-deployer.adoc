...
----
====
. Explore downloaded example custom values files in the `suse-ai-deployer/examples` subdirectory.
...
. Create `custom-overrides.yaml` custom values file based on one of the above examples.
...
Use the appropriate `RELEASE_NAME` and `SAIF_NAMESPACE` based the configuration in `custom-overrides.yaml`.
+
[source,subs="+attributes"]
----
