...
Assigning graphics processing unit (GPU) nodes ensures that containerized applications run on hardware equipped with GPU acceleration.
Matching demanding workloads, such as {ollama}, with specific worker nodes optimizes cluster resource allocation and performance.
Configure this matching by labeling target worker nodes and defining explicit resource requests in {kube}.
endif::[]
