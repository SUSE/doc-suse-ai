...
. Verify that the {owui} installation custom values file `owui_custom_overrides.yaml` includes the following content. 
+
[source,yaml]
----
pipelines:
...
--
---
. After you fill the custom values file with correct values, install or update {owui}. 
+
[source,bash]
----
> helm upgrade \
...
. Save the enhanced custom values file and update {owui}:
+
[source,bash]
----
> helm upgrade \
  --install open-webui oci://dp.apps.rancher.io/charts/open-webui \
  -n <replaceable>SAIF_NAMESPACE</replaceable> \
  --create-namespace \
  --version 7.2.0 \
  -f owui_custom_overrides.yaml
----
