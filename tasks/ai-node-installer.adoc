...
Target Host Requirements::
  * Supported Linux distribution - {sle}, {slm} or {leap} variants recommended.
  ...
  * Target hosts include python 3.11+ version.
...
Recommended for localhost is to manually install the drivers.
====

[#ai-node-installer-use-cases]
== `extra_vars.yml` examples tailored to specific use cases

.You already have OS provisioned on the servers and you want to deploy an {rke2a} cluster with {ranchera} management server.
====
