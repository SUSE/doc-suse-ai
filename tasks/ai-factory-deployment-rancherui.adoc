...
[#ai-factory-extension-github-branch]
== Installation using a GitHub branch (the recommended method)

The {saifactory} extension is primarily distributed via a GitHub branch (gh-pages).
...
. Enter `https://github.com/SUSE/suse-ai-factory` as the Git repository URL and `gh-pages` as the Git branch.
Confirm with btn:[Create].
. Wait until the {saifactory} repository has the Active state.
. Go back to the `Extensions` page and install {saifactory}.
. Reload {ranchera} Dashboard by reloading the Web browser window to ensure the extension is loaded in the UI.
. The "SUSE AI Factory" logo will now appear on the left panel of the {ranchera} Dashboard.
...
[#ai-factory-extension-github-registry]
...
.. Enter `ghcr.io/suse/suse-ai-factory:<VERSION>` in the btn:[Catalog Image Reference] input field.
.. Replace `<VERSION>` with a tag published in the GitHub Container Registry.
.. Confirm with btn:[Load].
. From the `Manage Repositories` page, verify if the {saifactory} repository has the `Active` state.
If not, refresh the connection.
. Go back to the `Extensions` page and start the installation by clicking btn:[{saifactory}].
. Reload {ranchera} Dashboard by reloading the Web browser window to ensure the extension is loaded in the UI.
The "`SUSE AI Factory`" logo will now appear on the left panel of the {ranchera} Dashboard.
