...
{helm} charts of AI applications are hosted in a private {suse} trusted registries {sappco} and {sregistry}.
...
. Refer to the document link:https://docs.apps.rancher.io/get-started/authentication[{ranchera} Authentication] to create the username and access token (password) for {sappco}.
...
.. If your organization purchased {productname} subscription, you can find the {productname} subscription and its registration key in the subscriptions list.
. In {ranchera}, navigate to menu:Cluster[Apps > Repositories].
. Click btn:[Create] and provide the following details:
