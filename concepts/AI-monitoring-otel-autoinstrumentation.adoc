...
This section describes how to instrument Kubernetes applications using the {otelemetry} Operator for Kubernetes.
It also explains how to forward telemetry (metrics and traces) to {sobservability}.
The Operator simplifies deployment of {otelemetry} components.
It also enables automatic instrumentation without modifying application code.
...
Install the {otelemetry} Operator into your cluster using the Rancher Application Collection Helm chart.
...
* `Instrumentation`: Defines automatic instrumentation settings and exporter configuration for workloads.
...
The following snippet shows a {suseai} example `otel-values.yaml`:
...
=== Example: enable auto-instrumentation in {suseai}
...
For example, check the OpenTelemetry Services and Traces views.
...
=== Configure image pulls from Application Collection

To pull the Collector image from Application Collection, create a ServiceAccount with `imagePullSecrets`.
...
* Resource enrichment: Kubernetes attributes (for example, `k8s.pod.name` and `k8s.namespace.name`) help {sobservability} correlate telemetry with topology.

[#ai-monitoring-otel-operator-troubleshooting]
== Troubleshooting

[#ai-monitoring-otel-operator-troubleshooting-common]
...
Cause::
A typo in the annotation, the wrong language annotation (for example, `inject-java` vs `inject-python`), or the `Instrumentation` resource not being present in the namespace at pod startup.
Fix::
Confirm the annotation matches the intended language.
...
Symptom::
Instrumentation defaults to `http://localhost:4317` if `spec.exporter.endpoint` is omitted.
Telemetry is dropped or sent to a pod-local endpoint.
Fix::
...
Symptom::
The Operator webhook rejects resources.
You can see error events for webhook certificates.
Fix::
