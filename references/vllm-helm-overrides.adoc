...
= Examples of {vllm} {helm} chart custom values files
endif::[]
// overridden abstract?
...
You can integrate {vllm} in {owui} either using the {owui} Web user interface, or updating {owui} custom values file during {owui} deployment (see xref:owui-ollama-deploy-vllm[]).
...
[source]
----
http://vllm-router-service.<SAIF_NAMESPACE>.svc.cluster.local:80/v1
----
+
...
====
The following custom values file installs {vllm} using a model that is publicly available.
...
.Basic configuration
====
The following {vllm} custom values file includes basic configuration options.
...
[source,bash,subs="+attributes"]
----
{prompt_user}kubectl create secret -n <SAIF_NAMESPACE> \
...
{prompt_user}kubectl -n <SAIF_NAMESPACE> \
  wait --for=condition=complete job/prefetch-llama3.1-8b
----

. Update the {vllm} custom values file with support for PVC.
+
[source,yaml,subs="+attributes"]
----
# vllm_custom_overrides.yaml
