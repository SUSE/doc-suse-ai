...
= {litellm} custom values examples
endif::[]
// overridden abstract?
...
== Basic custom values file with {postgresql} deployment and an automatically generated master key
...
----
{prompt_user}kubectl \
  -n {ai-app-string}-system port-forward service/litellm 4000:4000
----
...
{prompt_user}kubectl get secret litellm-masterkey \
  -n {ai-app-string}-system -o jsonpath='{.data.masterkey}' | base64 --decode
----
