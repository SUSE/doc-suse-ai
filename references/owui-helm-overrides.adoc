...
= Examples of {owui} {helm} chart custom values files
endif::[]
// overridden abstract?
...
.{owui} custom values file with {ollama} included
====
The values file installs {ollama} during the {owui} installation.
...
ollamaUrls:
- http://open-webui-ollama.<SAIF_NAMESPACE>.svc.cluster.local:11434
persistence:
  enabled: true
...
- name: VECTOR_DB
  value: "milvus"
- name: MILVUS_URI
  value: http://milvus.<SAIF_NAMESPACE>.svc.cluster.local:19530
...
.{owui} custom values file with {ollama} installed separately
====
The values file installs {ollama} separately from the {owui} installation.
...
global:
  ...
  - application-collection
ollamaUrls:
- http://ollama.<SAIF_NAMESPACE>.svc.cluster.local:11434
persistence:
  enabled: true
  storageClass: local-path <.>
ollama:
...
.{owui} custom values file with pipelines enabled
====
The values file installs {ollama} separately and enables {owui} pipelines.
...
.{owui} custom values file with a connection to {vllm}
====
The following example shows how to extend the `extraEnvVars` section of the {owui} custom values file to connect to {vllm}.
Replace `SAIF_NAMESPACE` with your {kube} namespace.

[TIP]
=====
Find more details about installing {vllm} in xref:vllm-helm-overrides[].
=====
...
- name: OPENAI_API_BASE_URL
  value: "http://vllm-router-service.<SAIF_NAMESPACE>.svc.cluster.local:80/v1"
- name: OPENAI_API_KEY
  value: "dummy" <.>
----
...
- name: OPENAI_API_BASE_URLS
  value: "http://open-webui-pipelines.<SAIF_NAMESPACE>.svc.cluster.local:9099;http://vllm-router-service.<SAIF_NAMESPACE>.svc.cluster.local:80/v1"
...
Following is an example of the `open-webui-pipelines-values.yaml` custom values file.
